...
fn partition(characters: &mut Vec<Vec<char>>) {
    characters[0][0] = 'O';

    //Works from top left to bottom right.
    for i in 0..characters.len() {
        for j in 0..characters[i].len() {
            check_char(characters, i, j);
        }
    }

    //Works from bottom right to top left
    for i in (0..characters.len()).rev() {
        for j in (0..characters[i].len()).rev() {
            check_char(characters, i, j);
        }
    }

    //Works from bottom left to top right
    for i in (0..characters.len()).rev() {
        for j in 0..characters[i].len() {
            check_char(characters, i, j);
        }
...
fn check_char(characters: &mut Vec<Vec<char>>, i: usize, j: usize) {
    let mut current_check: char;
    if characters[i][j] == 'P' {
        return;
    }
    ...
    //Checks above
    if i > 0 {
        current_check = characters[i-1][j];
        if current_check == 'O' {
            characters[i][j] = 'O';
            return;
        }
    ...
    //Checks left
    if j > 0 {
        current_check = characters[i][j-1];
        if current_check == 'O' {
            characters[i][j] = 'O';
            return;
        }
